...
def load_data(path ='helper/courses_catalog.csv'):
    ...
    try:
        ...
        exit()
def build_graph_unique(catalog):
    """Build the course prerequisite graph with metrics - single node per course with combined track info"""
    G = nx.DiGraph()
    
    # Process each unique course (already has combined track info)
    for idx, course in catalog.iterrows():
        # Get tracks as a cleaned list
        tracks = [t.strip() for t in course['track'].split(',')] if course['track'] != 'None' else ['None']
        ...
        G.add_node(
            ...
            tracks=tracks,  # Store all tracks as a list
...
def build_graph(catalog):
    ...
    # First expand the catalog to have one row per track combination
    expanded_catalog = []
    for idx, course in catalog.iterrows():
        # Split tracks and create separate entries for each
        tracks = [t.strip() for t in course['track'].split(',')] if course['track'] != 'None' else ['None']
        
        for track in tracks:
            new_course = course.copy()
            new_course['track'] = track
            expanded_catalog.append(new_course)
    
    # Convert back to DataFrame for easier handling
    expanded_catalog = pd.DataFrame(expanded_catalog)
    
    # Add nodes with attributes - create unique nodes for each track version
    for idx, course in expanded_catalog.iterrows():
